...
class Diversify(Algorithm):
    def __init__(self, args):
        ...
        if args.use_ema:
            ...
            for param in self.ema_bottleneck.parameters():
                param.requires_grad = False
    ...
    def update(self, data, opt):
        """Enhanced main update method with domain alignment and contrastive learning"""
        # Handle PyG Data objects
        ...
        if self.args.domain_align_weight > 0:
            ...
            if torch.any(source_mask) and torch.any(~source_mask):
                ...
                target_features = all_z[~source_mask]
                align_loss = self.compute_domain_alignment(source_features, target_features)
        ...
        if self.args.contrastive_weight > 0 and self.projection_head:
            ...
            contrastive_loss = F.cosine_embedding_loss(
                projections, 
                torch.roll(projections, 1, 0), 
                torch.ones(len(projections)).to(self.args.device)
            )
        # ================================= #
    ...
    def predict(self, x):
        ...
        if self.args.use_ema:
            ...
            bottleneck_out = self.ema_bottleneck(features)
            return self.ema_classifier(bottleneck_out)
    ...
    def compute_domain_alignment(self, source, target):
        """Compute domain alignment loss using MMD"""
        # Basic MMD implementation
        diff = source.mean(0) - target.mean(0)
        return torch.norm(diff, p=2)  # L2 distance between domain means
    # ========================================= #
