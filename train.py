        ...
        class EnhancedTemporalGCN(TemporalGCN):
            def __init__(self, *args, **kwargs):
                ...
                self.skip_conn = nn.Sequential(
                    nn.Linear(kwargs['input_dim'], self.output_dim),
                    nn.ReLU(),
                )
            ...
            def forward(self, x):
                ...
                # Original GNN processing
                gnn_out = super().forward(x)  # [batch, time, features]
                
                # LSTM processing
                batch_size = gnn_out.size(0)
                lstm_out, _ = self.lstm(gnn_out)  # [batch, time, lstm_hidden_size * directions]
                lstm_out = self.dropout(lstm_out)
                lstm_out = self.lstm_proj(lstm_out)  # [batch, time, output_dim]
                
                # Temporal aggregation using mean pooling
                gnn_out = lstm_out.mean(dim=1)  # [batch, output_dim]
