...
def get_curriculum_loader(args, algorithm, train_dataset, val_dataset, stage):
    """Advanced curriculum data loader with enhanced difficulty scoring"""
    ...
    domain_features = {}
    # Only call eval if it's available (handles our wrapped evaluator)
    ...
    if stage > 0 and logs.get('valid_acc') and logs['valid_acc'][-1] < 50:  # Low accuracy
    ...
    # Select samples ABOVE threshold (more challenging)
    selected_indices = [idx for idx, diff in sample_difficulties if diff >= difficulty_threshold]
    
    # If too few samples, expand threshold to include more challenging samples
    min_samples = max(100, int(0.3 * len(train_dataset)))
    if len(selected_indices) < min_samples:
        # Find the most difficult samples
        sample_difficulties.sort(key=lambda x: x[1], reverse=True)
        selected_indices = [idx for idx, _ in sample_difficulties[:min_samples]]
        print(f"Expanding curriculum to {len(selected_indices)} most difficult samples")
